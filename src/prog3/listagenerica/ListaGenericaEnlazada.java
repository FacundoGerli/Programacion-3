...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	private void llamadaRecursiva(ListaGenericaEnlazada<T> l, int i) {
		if(this.elemento(i)== null) {
			return;
		}
		else {
			llamadaRecursiva(l,i+1);
			l.agregarFinal(this.elemento(i));
		}
	}
	...
	public ListaGenericaEnlazada<T> invertir() {
	    int i = 0;
	    this.comenzar();
	    ListaGenericaEnlazada<T> recorrido = new ListaGenericaEnlazada<T>();
	    llamadaRecursiva(recorrido, i);
	    return recorrido;
	}
	public void imprimir() {
		this.comenzar();
		while(!this.fin()) {
			System.out.println(this.proximo()+" ");
		}
	}
	public ListaGenerica<T> clonar(){
		this.comenzar();
		ListaGenerica<T> nue = new ListaGenericaEnlazada();
		while(!this.fin()) {
			nue.agregarFinal(this.proximo());
		}
		return nue;
	}
